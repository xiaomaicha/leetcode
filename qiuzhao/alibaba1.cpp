...
SkipLinkList::SkipLinkList(int skipStep)
    : _skipStep(skipStep), _listHeader(NULL), _skipListHeader(NULL), _itemCount(0) {}
...
bool SkipLinkList::find(uint64_t key, uint64_t &value) {
  ...
  SkipLinkNode *current_skip = _skipListHeader;
  LinkNode *current_node;
  while (current_skip->next != nullptr && key > current_skip->next->pointer->key) {
    current_skip = current_skip->next;
  }
  current_node = current_skip->pointer;
  int i = 0;
  while (i<_skipStep && current_node != nullptr){
